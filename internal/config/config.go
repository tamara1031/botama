...
// NotifyConfig holds the runtime configuration for the notify module.
// Channels maps notification level names (e.g. "info", "warning") to Discord
// channel IDs and is populated dynamically from NOTIFY_<LEVEL>_CHANNEL_ID
// environment variables, so adding a new level requires no code change.
...
// loadNotifyChannels scans the process environment for every variable matching
// NOTIFY_<LEVEL>_CHANNEL_ID and returns a map of lowercase level → channel ID.
