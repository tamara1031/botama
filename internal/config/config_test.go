...
func TestLoad_NotifyChannelsEmptyByDefault(t *testing.T) {
	t.Setenv("DISCORD_TOKEN", "tok")
	t.Setenv("NOTIFY_INFO_CHANNEL_ID", "")
	t.Setenv("NOTIFY_WARNING_CHANNEL_ID", "")
	t.Setenv("NOTIFY_CRITICAL_CHANNEL_ID", "")
	...
	// Empty-value env vars must not produce map entries.
	for level, id := range cfg.Notify.Channels {
		if id == "" {
			t.Errorf("Channels[%q] should not be present with empty value", level)
		}
	}
}
